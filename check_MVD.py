def trivial_MVD(FD_dict,MVD,num_attrs): #MVD format: A->->B
    if num_attrs<=2:
        return True
    else:
        MVD=MVD.split('->->')
...
#-------------------------------------------------------------------------------------------
def define_MVDs(FD_dict,attrs_lst,num_attrs):
    print("The FD Dict in MVD is:",FD_dict)
    print("The attribute list in MVD is:",attrs_lst)
    print("The num_attr in MVD is:",num_attrs)
    var = input("Please enter MVDs in a string separated by comma {Format example: 'A->->B, D->->C'>}. ")
    var="".join(var.split())
    if var[0]=="'":
        var=var[1:]
    if var[-1]=="'":
